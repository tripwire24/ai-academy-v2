...
export default function ManageModulesPage({ params }: { params: { courseId: string } }) {
  ...
  useEffect(() => {
    const fetchCourseAndModules = async () => {
      try {
        const resCourse = await fetch(`/api/courses/${params.courseId}`);
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <div>
          <Link href={`/admin/courses/${params.courseId}`} className="inline-flex items-center gap-2 text-sm font-medium text-text-muted hover:text-text-primary transition-colors mb-4">
            ...
            modules.map((module, index) => (
              <div key={module.id} className="p-4 flex items-center grid grid-cols-12 gap-4 hover:bg-surface-elevated/50 transition-colors group">
                ...
                <div className="col-span-1 flex items-center justify-end gap-2">
                  <Link 
                    href={`/admin/courses/${params.courseId}/modules/${module.id}`}
